...
import { ArrowLeft, Loader2, Shield, Trophy, Swords } from "lucide-react";
...
const KNOCKOUT_STAGES = [
  { key: "r16",   label: "Dieciseisavos de Final", short: "1/32" },
  { key: "r32",   label: "Octavos de Final",        short: "1/16" },
  ...
  return (
    <div className={`glass rounded-xl p-3 border ${finished ? "border-primary/20" : "border-border/30"}`}>
      ...
      <div className="flex items-center gap-2 mb-1.5">
        ...
        {finished && (
          <span className={`font-mono font-bold text-sm flex-shrink-0 w-5 text-right ${homeWins ? "text-primary" : "text-muted-foreground"}`}>
            {match.home_score}
          </span>
        )}
      </div>
      ...
      <div className="flex items-center gap-2">
        ...
        {finished && (
          <span className={`font-mono font-bold text-sm flex-shrink-0 w-5 text-right ${awayWins ? "text-primary" : "text-muted-foreground"}`}>
            {match.away_score}
          </span>
        )}
      </div>
      ...
      {/* Badge final */}
      {finished && (
        <div className="mt-2 text-[9px] font-mono uppercase tracking-widest text-secondary">Final</div>
      )}
    </div>
  );
}
...
function KnockoutBracket({ matches }: { matches: Match[] }) {
  ...
  // Ordenar cada etapa por kickoff
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <h2 className="font-display font-bold text-3xl">
            Cuadro de <span className="text-gradient-hero">eliminatorias</span>
          </h2>
        </div>
      </div>
            ...
            <div key={stage.key}>
              ...
              <div className="flex items-center gap-3 mb-3">
                <div className={`px-3 py-1 rounded-full text-xs font-mono font-bold ${
                  isFinal ? "bg-gold/20 text-gold" : "bg-primary/10 text-primary"
                }`}>
            ...
            {knockoutQ.isLoading ? (
              ...
              <div className="mt-12 glass rounded-2xl p-8 text-center border border-border/30">
                ...
                <p className="text-sm text-muted-foreground">
                  El cuadro de eliminatorias aparece acá cuando el admin cargue los octavos de final.
                </p>
              </div>
            )}
          </>
