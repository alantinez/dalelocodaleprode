...
export const Route = createFileRoute("/stats")({
  ...
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Estadísticas avanzadas del Prode del Mundial 2026." },
    ],
  }),
});
...
type PlayerRow = {
  ...
  total_points: number; exact_hits: number; current_streak: number;
};

type PredRow = {
  id: string; user_id: string; match_id: string;
  home_score: number; away_score: number; points: number; is_exact: boolean;
  matches: {
...
function Kpi({ icon: Icon, label, value, tone = "primary" }: {
  icon: typeof Trophy; label: string; value: string | number;
  tone?: "primary" | "secondary" | "gold" | "destructive";
}) {
  const tones: Record<string, string> = {
    primary: "text-primary", secondary: "text-secondary",
    gold: "text-gold", destructive: "text-destructive",
  };
  return (
...
function PlayerSelect({ players, value, onChange, label }: {
  players: PlayerRow[]; value: string; onChange: (v: string) => void; label: string;
}) {
  return (
...
function computeStats(preds: PredRow[]) {
  ...
  let cum = 0;
  const timeline = ordered.map((p) => {
    cum += p.points ?? 0;
    return { day: p.matches!.kickoff.slice(5, 10), pts: cum };
  });
  let streak = 0, bestStreak = 0;
  for (const p of ordered) {
    if ((p.points ?? 0) > 0) { streak += 1; bestStreak = Math.max(bestStreak, streak); }
    else streak = 0;
  }
...
function StatsPage() {
  ...
  const predsAQ = useQuery({
    queryKey: ["stats-preds", activeA], enabled: !!activeA,
    queryFn: async () => {
  ...
  const predsBQ = useQuery({
    queryKey: ["stats-preds", activeB], enabled: !!activeB,
    queryFn: async () => {
      const { data, error } = await supabase.from("predictions")
        .select(`id,user_id,match_id,home_score,away_score,points,is_exact,
          matches:matches!predictions_match_id_fkey(id,kickoff,status,home_score,away_score,home_team_id,away_team_id)`)
  ...
  const h2h = useMemo(() => {
    ...
    const rows = (predsAQ.data ?? []).map((a) => ({ a, b: mapB.get(a.match_id) }))
      .filter((r): r is { a: PredRow; b: PredRow } => !!r.b);
    const sameCall = rows.filter((r) => r.a.home_score === r.b.home_score && r.a.away_score === r.b.away_score).length;
  ...
  const combinedTimeline = useMemo(() => {
    ...
    statsA.timeline.forEach((t) => map.set(t.day, { day: t.day, a: t.pts }));
    statsB.timeline.forEach((t) => { const e = map.get(t.day) ?? { day: t.day }; e.b = t.pts; map.set(t.day, e); });
      ...
      <main className="pt-28 pb-24 mx-auto max-w-6xl px-4 sm:px-6">
        <header className="mb-8">
          ...
          <p className="text-muted-foreground mt-2 max-w-2xl">
            Compará tu rendimiento con cualquier jugador del Prode.
          </p>
        </header>

        <div className="glass-strong rounded-2xl p-4 sm:p-5 mb-6 flex flex-wrap gap-3 items-end">
          <PlayerSelect players={players} value={activeA} onChange={setSelA} label="Jugador A" />
          <Swords className="hidden sm:block w-6 h-6 text-muted-foreground mb-2" />
          <PlayerSelect players={players} value={activeB} onChange={setSelB} label="Jugador B (opcional)" />
        </div>
          ...
          <>
            <section className="mb-8">
              <div className="flex items-center gap-3 mb-3">
                <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-primary to-secondary flex items-center justify-center overflow-hidden">
                  {playerA.avatar_url ? <img src={playerA.avatar_url} alt="" className="w-full h-full object-cover" /> : <Trophy className="w-5 h-5 text-background" />}
                </div>
              ...
              <div className="grid sm:grid-cols-2 gap-3 mt-3">
                <div className="glass rounded-2xl p-4">
                  <div className="text-xs font-mono uppercase tracking-widest text-secondary">Mejor fecha</div>
                  <div className="font-display font-bold text-xl mt-1">{statsA.best ? `${statsA.best.day} · +${statsA.best.pts} pts` : "—"}</div>
                </div>
                <div className="glass rounded-2xl p-4">
                  <div className="text-xs font-mono uppercase tracking-widest text-destructive">Peor fecha</div>
                  <div className="font-display font-bold text-xl mt-1">{statsA.worst ? `${statsA.worst.day} · ${statsA.worst.pts} pts` : "—"}</div>
                </div>
              </div>
            </section>
            ...
            <section className="glass-strong rounded-2xl p-5 mb-8">
              ...
              {combinedTimeline.length === 0 ? (
                <div className="text-sm text-muted-foreground py-10 text-center">Sin partidos finalizados todavía.</div>
              ) : (
                <div className="h-64">
                  <ResponsiveContainer>
                    <LineChart data={combinedTimeline}>
                      ...
                      <Tooltip contentStyle={{ background: "oklch(0.18 0.02 250)", border: "1px solid oklch(0.3 0.02 250)", borderRadius: 12 }} />
                      <Legend />
                      <Line type="monotone" dataKey="a" name={playerA.display_name} stroke="oklch(0.7 0.2 240)" strokeWidth={3} dot={false} />
                      {activeB && <Line type="monotone" dataKey="b" name={playerB?.display_name ?? "B"} stroke="oklch(0.82 0.17 85)" strokeWidth={3} dot={false} />}
                    </LineChart>
                  </ResponsiveContainer>
                </div>
              )}
                ...
                {h2h && (
                  <div className="mt-4 glass rounded-xl p-4 flex items-center gap-3">
                    ...
                    <div className="text-sm">
                      ...
                      <span className="text-muted-foreground">predicciones idénticas en {h2h.rows.length} partidos en común</span>
                    </div>
                  </div>
                )}
              </section>
